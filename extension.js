(function () {
    ...
    function extend() {
        ...
        bot.commands.hledatCommand = {
            ...
            bot.commands.kamenCommand = {
            command: 'kamen',  //The command to be called. With the standard command literal this would be: !bacon
            rank: 'user', //Minimum user permission to use the command
            type: 'startsWith', //Specify if it can accept variables or not (if so, these have to be handled yourself through the chat.message
            functionality: function (chat, cmd) {
                if (this.type === 'exact' && chat.message.length !== cmd.length) return void (0);
                if (!bot.commands.executable(this.rank, chat)) return void (0);
                    ...
                    for (var i = 0; i < bot.room.users.length; i++) {
                        if (bot.room.users[i].username === searchUser) {
                            found = bot.room.users[i];
                            if (found.inRoom) {
                                API.sendChat("/me @" + chat.un + " dal/a kámen @" +searchUser+ ".");
                            } else {
                                API.sendChat("/me [@" + chat.un + "] Nemůžeš hrát kámen, nůžky, papír s " + searchUser + ", protože se odpojil/a před " + bot.roomUtilities.msToStr(new Date().getTime() - found.lastActivity)+ "");
                            }
                            break;
                        }
                    }
                    if (!found) {
                        API.sendChat("/me Nemůžeš hrát hru s "+searchUser+". Buď neexistuje nebo se odpojil/a.")
                    }
                }
            }
        };
        bot.commands.nuzkyCommand = {
            command: 'nuzky',  //The command to be called. With the standard command literal this would be: !bacon
            rank: 'user', //Minimum user permission to use the command
            type: 'startsWith', //Specify if it can accept variables or not (if so, these have to be handled yourself through the chat.message
            functionality: function (chat, cmd) {
                if (this.type === 'exact' && chat.message.length !== cmd.length) return void (0);
                if (!bot.commands.executable(this.rank, chat)) return void (0);
                    ...
                    for (var i = 0; i < bot.room.users.length; i++) {
                        if (bot.room.users[i].username === searchUser) {
                            found = bot.room.users[i];
                            if (found.inRoom) {
                                API.sendChat("/me @" + chat.un + " dal/a nůžky @" +searchUser+ ".");
                            } else {
                                API.sendChat("/me [@" + chat.un + "] Nemůžeš hrát kámen, nůžky, papír s " + searchUser + ", protože se odpojil/a před " + bot.roomUtilities.msToStr(new Date().getTime() - found.lastActivity)+ "");
                            }
                            break;
         ...
         bot.commands.papirCommand = {
            command: 'papir',  //The command to be called. With the standard command literal this would be: !bacon
            rank: 'user', //Minimum user permission to use the command
            type: 'startsWith', //Specify if it can accept variables or not (if so, these have to be handled yourself through the chat.message
            functionality: function (chat, cmd) {
                if (this.type === 'exact' && chat.message.length !== cmd.length) return void (0);
                if (!bot.commands.executable(this.rank, chat)) return void (0);
                    ...
                    for (var i = 0; i < bot.room.users.length; i++) {
                        if (bot.room.users[i].username === searchUser) {
                            found = bot.room.users[i];
                            if (found.inRoom) {
                                API.sendChat("/me @" + chat.un + " dal/a papír @" +searchUser+ ".");
                            } else {
                                API.sendChat("/me [@" + chat.un + "] Nemůžeš hrát kámen, nůžky, papír s " + searchUser + ", protože se odpojil/a před " + bot.roomUtilities.msToStr(new Date().getTime() - found.lastActivity)+ "");
                            }
                            break;
