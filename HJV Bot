...
(function () {
    ...
    var loadChat = function (cb) {
        ...
        $.get("https://raw.githubusercontent.com/Franta72/BOT---Czech/master/langIndex.json", function (json) {
...
afkCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!basicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        var space = msg.indexOf(' ');
                        if (space === -1) {
                            API.sendChat(basicBot.chat.giveafk);
                            return false;
                        }
                        else {
                            var name = msg.substring(space + 2);
                            var user = basicBot.userUtilities.lookupUserName(name);
                            if (user === false || !user.inRoom) {
                                return API.sendChat(subChat(basicBot.chat.nouserafk, {name: name}));
                            }
                            else if (user.username === chat.un) {
                                return API.sendChat(subChat(basicBot.chat.selfafk, {name: name}));
                            }
                            else {
                                return API.sendChat(subChat(basicBot.chat.giveafk, {nameto: user.username, namefrom: chat.un, AFK: this.getafk()}));
                            }
                        }
                    }
                }
                    ...
                    if (!basicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (space === -1) {
                            ...
                            if (user === false || !user.inRoom) {
                                ...
                                return API.sendChat(subChat(basicBot.chat.givezpet, {nameto: user.username, namefrom: chat.un, zpet: this.getzpet()}));
                            }
                        }
                    }
                }
