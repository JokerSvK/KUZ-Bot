...
(function () {
    ...
    var basicBot = {
        ...
        settings: {
            ...
            skipPosition: 1,
            skipReasons: [
                ["akce", "tvá píseň není vhodná k tématu naší akce. "],
                ...
                ["zvuk", "tvá píseň měla špatný zvukový záznam. "],
                ["nsfw", "tvá píseň měla nevhodný obsah pro děti a mladistvé. "],
                ["nejde", "tvá píseň nešla přehrát pro ostatní uživatelé. "]
            ],
            afkpositionCheck: 15,
            ...
            sluzbyCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!basicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        var space = msg.indexOf(' ');
                        if (space === -1) {
                            API.sendChat(basicBot.chat.givesluzby);
                            return false;
                        }
                        else {
                            var name = msg.substring(space + 2);
                            var user = basicBot.userUtilities.lookupUserName(name);
                            if (user === false || !user.inRoom) {
                                return API.sendChat(subChat(basicBot.chat.nousersluzby, {name: name}));
                            }
                            else if (user.username === chat.un) {
